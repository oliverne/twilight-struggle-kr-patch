...
- **상태**: ✅ 처리 완료 (2026-08-13, Phase 7) — 씬 규칙 문단 167개 전량 한글화·실게임 확인 ([phase-7-help-translation.md](phases/phase-7-help-translation.md))
...
- **확정**: ✅ 4차 실게임 테스트에서 카드/스코어링/우주경쟁/DEFCON/HELP 도움말 모두 한글 확인 (2026-08-12)
- **재발 방지 보강 (2026-08-12)**: add_ko_columns.py 검증 강화 — ① KO 열이 원본 존재 열 범위 내인지 자동 검사 (범위 밖이면 즉시 실패) ② 키 열(1열) 보존 검사 ③ **멱등성 버그 수정** — 기존 KO 열 제거 후 헤더 dict 미갱신으로 재실행 시 KO 열이 한 칸씩 밀리던 문제(9→10→11...) 발견·수정. 반복 실행해도 9/8/8/8/10열로 고정됨
- **관련 Phase**: Phase 5
...
### #18 "턴 및 행동트랙" 첫 칸 H가 칸을 벗어남 → ✅ 해결 (2026-08-13)
- **증상**: 인게임 팝업 "턴 및 행동트랙"에서 미국/소련 턴 트랙 9칸 중 첫 칸(H, 헤드라인)만 레이아웃을 벗어나 표시. 1~8은 정상
- **원인**: 트랙 라벨의 `IMPACT SDF`가 Paperlogy로 교체되면서 H 글리프 폭이 숫자보다 20% 넓어짐 — 원본 Impact는 H(55.5% em)≈8(53.5% em)로 동일 폭이었음
- **해결**: `apply_font_mapping.py`에서 IMPACT SDF/NUM Outline/Shadow 3종을 NotoSerifKR(D2Coding 모노스페이스, H=1=8 동일 폭 34.5)로 이동 + 재주입 + 설치 (`7ecf94e`)
