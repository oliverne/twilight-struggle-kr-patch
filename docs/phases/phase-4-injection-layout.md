...
> ⚠️ **2026-08-14 개정 — EN 로케일 덮어쓰기 방식**: 이 문서의 주입 방식(Common_Strings는 RU→KO 교체 + 언어=KO 설정)은 폐기됐다.
> 현재는 **모든 언어 테이블의 EN 열에 한글을 직접 주입**하고 게임 언어 설정은 건드리지 않는다 (설치=한글, 제거=영어).
> 최신 절차: `scripts/inject_translations.py --platform windows|macos` → `patched/<플랫폼>/` 참조.
> 아래 내용은 당시 실행 기록이다.
