...
- 검은 마스킹 리빌: 스크롤 진입 시 마스킹 바가 벗겨져 정보가 해제된다
...
- **서류 02(입수) + 철 문서함(설치)**: 데스크톱에서 7fr/5fr 그리드. 철 문서함은 sticky(top 4.5rem)로 스크롤 내내 보인다.
- **반응형**: 880px 이하에서 단일 컬럼, 철 문서함 sticky 해제. 560px 이하에서 OS 카드가 2열(이름+파일명 적층), 다운로드 패널이 세로 적층.
...
- **스크린샷 슬롯**: 16:9 플레이트, 48px 대각선 해칭, 코너 레지스트레이션 마크(등록 표식), '미첨부' 미니 도장.
...
### 철 문서함 (설치 카드)
- **Style:** navy-panel 배경 · navy-border 테두리 · paper 섀도 · sticky(데스크톱). 단계 = Oswald 번호(red-bright) + Paperlogy 제목 + steel 본문, 점선 구분.
- **명령 블록:** navy-cover 배경 · 좌측 3px 빨간 바 · 모노 0.74rem · 가로 스크롤 + 복사 버튼(steel → 호버 red).

### 마스킹 리빌 (시그니처 컴포넌트)
- 서류 시트 진입 시 검은 마스킹 바(하단 들쭉날쭉 clip-path)가 0.9s ease-out으로 위로 벗겨지고, 콘텐츠가 0.35s 지연 후 페이드인. prefers-reduced-motion에서는 즉시 표시.

### 스크린샷 슬롯
- 16:9 플레이트 · paper-deep 해칭 · '미첨부' 미니 도장 + "교체 예정" 라벨 — 실제 스크린샷으로 교체하기 전까지 정직한 자리 표시.
